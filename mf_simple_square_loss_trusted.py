...
4. Add Laplacian noise to the gradient
...
class MFModel(object):
  ...
  def fit(self,private_mode,privacy_budget,train_rating_matrix, learning_rate):
    ...
    for i in range(num_examples):
      (user, item, rating,privacy_weight) = train_rating_matrix[i]
      ...
      if private_mode==0:
        ...
        err_ui=privacy_weight*rating-prediction
        for k in range(embedding_dim):
...
def main():
  ...
  parser.add_argument('--privacy_budget', type=float, default=1.0,
                      help='Overall privacy budget for the system')

  parser.add_argument('--nonprivate_epochs', type=int, default=30,
                      help='Number of non-private training epochs')

  parser.add_argument('--private_epochs', type=int, default=30,
                      help='Number of non-private training epochs')
  ...
  for i in range(len(train_rating_matrix)):
    ...
    train_rating_matrix[i].append(rating_privacy_weight)
  ...
  print('Epoch %4d:\t MSE=%.4f\t'
        % (0, mse))

  for epoch in range(args.nonprivate_epochs):
    # Non_private Training
    private_mode=0
    _ = model.fit(private_mode,args.privacy_budget,train_rating_matrix, learning_rate=args.learning_rate)
